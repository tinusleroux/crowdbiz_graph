...
-- Purpose: Eliminate complex joins and improve dashboard performance
-- ==================================================================

-- 1. NETWORK STATUS TABLE (Person Summary)
-- This replaces the complex person enrichment queries
...
CREATE INDEX network_status_name_idx ON network_status USING GIN (to_tsvector('simple', full_name));
CREATE INDEX network_status_org_idx ON network_status (current_organization);
CREATE INDEX network_status_org_type_idx ON network_status (current_org_type);
CREATE INDEX network_status_sport_idx ON network_status (current_sport);
CREATE INDEX network_status_executive_idx ON network_status (is_executive) WHERE is_executive = TRUE;
CREATE INDEX network_status_updated_idx ON network_status (last_updated);

-- 2. ORGANIZATION SUMMARY TABLE 
-- This replaces complex organization queries and employee counting
CREATE TABLE organization_summary (
    ...
    sport TEXT,
    industry TEXT,
    parent_org_name TEXT, -- denormalized parent organization name
    ...
    departments JSONB DEFAULT '{}', -- {"Sales": 5, "Marketing": 3, ...}
    
    -- Role Distribution
    role_distribution JSONB DEFAULT '{}', -- {"Manager": 2, "Director": 1, ...}
...
-- Indexes for efficient querying
CREATE INDEX org_summary_name_idx ON organization_summary USING GIN (to_tsvector('simple', name));
CREATE INDEX org_summary_org_type_idx ON organization_summary (org_type);
CREATE INDEX org_summary_sport_idx ON organization_summary (sport);
CREATE INDEX org_summary_employees_idx ON organization_summary (current_employees);
CREATE INDEX org_summary_executive_idx ON organization_summary (executive_count);
CREATE INDEX org_summary_updated_idx ON organization_summary (last_updated);

-- ==================================================================
-- REFRESH FUNCTIONS
-- ==================================================================

-- Function to refresh network_status table
CREATE OR REPLACE FUNCTION refresh_network_status()
RETURNS INTEGER AS $$
DECLARE
    updated_count INTEGER := 0;
BEGIN
    -- Truncate and rebuild (for now - can be optimized to incremental updates)
    TRUNCATE network_status;
...
-- Function to refresh organization_summary table
...
SELECT refresh_network_status();
SELECT refresh_organization_summary();

-- ==================================================================
-- COMMENTS AND DOCUMENTATION
-- ==================================================================

COMMENT ON TABLE network_status IS 'Denormalized person summary table for dashboard performance - eliminates complex joins';
COMMENT ON TABLE organization_summary IS 'Organization metrics and employee counts - updated daily';

COMMENT ON FUNCTION refresh_network_status() IS 'Rebuilds network_status table with latest person and role data';
COMMENT ON FUNCTION refresh_organization_summary() IS 'Rebuilds organization_summary table with employee counts and metrics';
